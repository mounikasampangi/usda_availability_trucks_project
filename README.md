...
- **Bands tighten near the middle.** Q4 readings cluster far closer to adequate than Q3 ones do. Half-point bands would flatten most of the country into a single gray and hide a real 1.3-point spread, so the steps narrow to 0.4 either side of 3. The cuts are fixed rather than fitted to the current selection, so a color means the same thing in every year and month.
